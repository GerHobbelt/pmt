...
png_add_chunk(unsigned char* buffer, int size, png_chunk* chunks, int chunksize)
{
	...
	{
		add_chunksize += (4 + 4 + chunks[i].length + 4);
	}
...
png_remove_chunk(unsigned char* buffer, int size, int* chunks, int chunksize)
{
		...
		res = png_parser_next(&state, &chunk))
	{
		...
		if (i == chunks[j])
		{
			// save the ptr because we will move the chunk behind this
			// to here
			mvptr = chunk.data - 8;
			j++;
			continue;
...
int
png_remove_chunk_by_type(unsigned char* buffer, int size, char** type, int typesize)
{
	png_state state = { 0 };
	png_chunk chunk = { 0 };
	if (png_parser_create(&state, buffer, size) != PNG_OK)
